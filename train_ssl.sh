...
cp -R $WRKDIR/FixMatch  /tmp/$SLURM_JOB_ID
cp $WRKDIR/models.py tmp/$SLURM_JOB_ID/FixMatch  # Copy the models file to the supervised folder
cp $WRKDIR/ViT-B_16.npz tmp/$SLURM_JOB_ID/FixMatch  # Copy the pretrained weights to the supervised folder
cd /tmp/$SLURM_JOB_ID
